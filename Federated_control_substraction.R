#The following packages can be loaded if already installed. 
#If they are not installation should be done first with "install.packages('desired_package')"
...
#This is a script to extract the external control samples from your cohort data
#Please put all different OLINK batches in one directory and insert the path of that directory in line 286

#first define the read_NPX function (OLINK)
...
#Define the control isolation function.
control_isolation <- function(df){
  control_id <- df %>% filter(grepl("control", SampleID, ignore.case = T))
}


#Apply control_isolation function on the data.
for (i in 1:length(df)) { 
controls_substracted <- control_isolation(as.data.frame(df[i]))
df[[i]] <- controls_substracted
}

#Write the controls out as an RDataset with the name of your cohort as file name and upload the file to the SFTP server.
save(df, file = "controls_yourcohortname.RData")
